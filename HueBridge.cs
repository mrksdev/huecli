...
namespace huecli
{
    ...
    class HueBridge
    {
        ...
        public void TurnOnLighting(string lightID)
        {
            ...
            HttpResponseMessage responseMessage = apiClient.PutAsync(this.GetBaseUrl(lightID), content).Result;
        }
        ...
        public void SetLightingColor(string lightID, string hue)
        {
            HttpClient apiClient = new HttpClient();
            StringContent content = new StringContent("{\"hue\": "+hue+"}", Encoding.UTF8, "application/json");
            HttpResponseMessage responseMessage = apiClient.PutAsync(this.GetBaseUrl(lightID), content).Result;
        }

        public void SetLightingEffect(string lightID, string effect = "none")
        {
            HttpClient apiClient = new HttpClient();
            StringContent content = new StringContent("{\"effect\": \""+effect+"\"}", Encoding.UTF8, "application/json");
            HttpResponseMessage responseMessage = apiClient.PutAsync(this.GetBaseUrl(lightID), content).Result;
        }

        private string GetBaseUrl(string lightId)
        {
            return $"http://{this.HueBridgeAddress}/api/{this.HueBridgeUsername}/lights/{lightId}/state";
        }
    }
}